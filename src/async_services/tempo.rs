...
pub struct AsyncTrackBpmService {
    socket: UdpSocket,
    buffer: [u8; 1024],
}

impl AsyncTrackBpmService {
    pub async fn new() -> ProDjLinkResult<Self> {
        let socket = UdpSocket::bind((Ipv4Addr::UNSPECIFIED, BEAT_PORT)).await?;
    ...
    pub async fn recv(&mut self) -> ProDjLinkResult<Option<TimingPacket>> {
